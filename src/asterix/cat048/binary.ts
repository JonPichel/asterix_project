...
import { checkFX, octalToDecimal, twosComplement } from "../utils"
import { processID } from "./aircraftID"
import { gpsCoords } from "./coords"
import DataRecord048 from "./record"

export function fromBinary(buffer: Uint8Array): DataRecord {
  ...
  const record: DataRecord048 = { category: 48 }
  ...
  // Get GPS coordinates
  const coords = gpsCoords(record)
  if (coords !== null) {
    record.gpsCoords = coords
  }
  return record
}
...
function parse140(record: DataRecord048, buffer: Uint8Array): number {
  record.secondsSinceMidnight =
    (((buffer[0] << 16) | (buffer[1] << 8) | buffer[2]) * 1) / 128
  const today = new Date()
  const millisToday = Date.UTC(
    today.getFullYear(),
    today.getMonth(),
    today.getDate(),
    -1,
  ...
  record.timestamp = Math.round(
    millisToday + record.secondsSinceMidnight * 1000
  )
  return 3
}
...
function parse070(record: DataRecord048, buffer: Uint8Array): number {
  ...
  if (((buffer[0] >> 5) & 0b1) == 0) {
    ...
    record.mode3ACode = octalToDecimal(
      ((buffer[0] & 0b1111) << 8) | buffer[1],
      4
    )
  }
...
function parse090(record: DataRecord048, buffer: Uint8Array): number {
  ...
  record.flightLevel =
    twosComplement(((buffer[0] & 0b111111) << 8) | buffer[1], 14) * 0.25 // FL
    ...
    if (bds2 == 0) {
      if (bds1 == 4) {
        if (((data[0] >> 7) & 0b1) === 1) {
          ...
          //       000BBBBB
          // 0AAAA AAABBBBB
          record.bds40MCPFCUSelectedAltitude =
            (((data[0] & 0b01111111) << 5) | (data[1] >> 3)) * 16 // ft
        }
        if (((data[1] >> 2) & 0b1) === 1) {
          ...
          record.bds40FMSSelectedAltitude =
            (((data[1] & 0b11) << 10) | (data[2] << 2) | (data[3] >> 6)) * 16 // ft
        }
        if (((data[3] >> 5) & 0b1) === 1) {
          // XXXAAAAA BBBBBBB0
          // 000AAAAA A0000000
          //          0BBBBBBB
          // 000AAAAA ABBBBBBB
          record.bds40BarometricPressureSetting =
            +((((data[3] & 0b11111) << 7) | (data[4] >> 1)) * 0.1).toFixed(3) +
            800 // mb
          if (
            record.flightLevel != undefined &&
            record.bds40BarometricPressureSetting != undefined &&
            record.bds40MCPFCUSelectedAltitude != undefined
          ) {
            if (record.flightLevel * 100 <= 6000) {
              record.correctedFlightLevel =
                +(
                  record.flightLevel +
                  ((record.bds40BarometricPressureSetting - 1013.25) * 30) / 100
                ).toFixed(3) * 100
            }
          }
        }
        ...
        if (((data[0] >> 7) & 0b1) === 1) {
          ...
          if (sign == -1) {
            record.bds50Roll = +(
              sign *
              (90 - ((((data[0] & 0b111111) << 3) | (data[1] >> 5)) * 45) / 256)
            ).toFixed(3) // º
          } else {
            record.bds50Roll = +(
              (sign * (((data[0] & 0b111111) << 3) | (data[1] >> 5)) * 45) /
              256
            ).toFixed(3) // º
          }
        }
        if (((data[1] >> 4) & 0b1) === 1) {
          ...
          if (sign == -1) {
            record.bds50TrueTrack = +(
              sign *
              (180 - ((((data[1] & 0b111) << 7) | (data[2] >> 1)) * 90) / 512)
            ).toFixed(3) // º
          } else {
            record.bds50TrueTrack = +(
              (sign * (((data[1] & 0b111) << 7) | (data[2] >> 1)) * 90) /
              512
            ).toFixed(3) // º
          }
        }
        ...
        if (((data[4] >> 5) & 0b1) === 1) {
          ...
          if (sign == -1) {
            record.bds50TrackRate = +(
              sign *
              (16 - ((((data[4] & 0b1111) << 5) | (data[5] >> 3)) * 8) / 256)
            ).toFixed(3) // º/s
          } else {
            record.bds50TrackRate = +(
              (sign * (((data[4] & 0b1111) << 5) | (data[5] >> 3)) * 8) /
              256
            ).toFixed(3) // º/s
          }
        }
        ...
        if (((data[0] >> 7) & 0b1) === 1) {
          const sign = ((data[0] >> 6) & 0b1) === 1 ? -1 : 1
          if (sign == -1) {
            record.bds60MagneticHeading = +(
              sign *
              (180 -
                ((((data[0] & 0b111111) << 4) | (data[1] >> 4)) * 90) / 512)
            ).toFixed(3) // º
          } else {
            record.bds60MagneticHeading = +(
              ((((data[0] & 0b111111) << 4) | (data[1] >> 4)) * 90) /
              512
            ).toFixed(3) // º
          }
        }
        ...
        if (((data[4] >> 5) & 0b1) === 1) {
          const sign = ((data[4] >> 4) & 0b1) === 1 ? -1 : 1
          if (sign == -1) {
            record.bds60BarometricAltitudeRate =
              sign *
              (16384 -
                ((((data[4] & 0b1111) << 5) | (data[5] >> 3)) * 8192) / 256) // ft/min
          } else {
            record.bds60BarometricAltitudeRate =
              ((((data[4] & 0b1111) << 5) | (data[5] >> 3)) * 8192) / 256 // ft/min
          }
        ...
        if (((data[5] >> 2) & 0b1) === 1) {
          ...
          if (sign == -1) {
            record.bds60InertialVerticalVelocity =
              sign * (16384 - ((((data[5] & 0b1) << 8) | data[6]) * 8192) / 256) // ft/min
          } else {
            record.bds60InertialVerticalVelocity =
              ((((data[5] & 0b1) << 8) | data[6]) * 8192) / 256 // ft/min
          }
        }
      }
    }
...
function parse042(record: DataRecord048, buffer: Uint8Array): number {
  record.Xcomponent =
    (twosComplement((buffer[0] << 8) | buffer[1], 16) * 1) / 128
  record.Ycomponent =
    (twosComplement((buffer[2] << 8) | buffer[3], 16) * 1) / 128
  return 4
}
